        ...
        while True:
            ...
            for i, frame_offset in enumerate(range(len(frames) - delta)):
                if (i + (n_frame-batch_size)) in frame_idx:
                    file_name = f"{Path(vid_file).parent.stem}_"f"{Path(vid_file).stem}_"f"frame_{i+(n_frame-batch_size):06d}.png"
                    frame_image = np.dstack([
                                frames[i] / 255, # grayscale original frame
                                blurred_frames[i], # foreground mask
                                np.abs(blurred_frames[i+delta] - blurred_frames[i]) # motion mask
                                ]).astype(np.float32)
...
def argument_parser():
    ...
    parser.add_argument("--json_path", default="Data/train_data/extracted_frames.json", help="Location of json file with frame_idx.")
    parser.add_argument("--out_dir", default="Data/train_data/bg_sub/", help="Output location for converted frames.")
    return parser
