...
4. **Edit the bash script if required**
    ...
    - `VIA_TRACKS_GLOB_PATTERN`: glob pattern to match the input VIA track files in the provided VIA tracks directory. By default, it is set to `"$VIDEO_NAME_NO_EXT*.csv"`.
    - `ZARR_STORE_OUTPUT`: path to the output directory for the zarr dataset. By default, it is set to a filename that includes the SLURM array job ID. This ensures that different run will generate different zarr datasets.
    - `ZARR_STORE_MODE`: determines whether to create a new zarr store (`'w'`) or to update an existing zarr store (`'a'`). By default it is set to `'a'`. We usually want to update an existing zarr store, since each job in the array processes files from a single video and we want to add the data for each video to the same zarr store.
    - `ZARR_MODE_GROUP`: determines whether to overwrite existing groups in the zarr store (`'w'`), update them (`'a'`), or throw an error if the group already exists (`'w-'`). By default it is set to `'w-'`. We usually want to throw an error if the group already exists, since each job in the array should create a new group for its corresponding video.
...
5.  **Run the job using the SLURM scheduler**
    ...
    ```
    sbatch <path-to-create-zarr-dataset-bash-script>
    ```
...
6. **Expected output**
    If the array job runs successfully, the expected output is a zarr dataset named `CrabTracks-slurm<SLURM_ARRAY_JOB_ID>.zarr` in the specified output directory. Each group in the zarr dataset corresponds to a video, and contains a `movement` [bounding box dataset](https://movement.neuroinformatics.dev/latest/user_guide/movement_dataset.html) for all tracks in the video.

    To inspect this dataset, please refer to the example notebooks in the 🦀 repository.

7. **Dealing with failed jobs**
    Sometimes some of the jobs in the array job fail due to non reproducible issues with `miniconda` in the cluster. This leads to an incomplete zarr store `store_1`, that contains only a subset of the videos we wanted to process. In those cases, this is the recommended approach to re-run the failed jobs and merge the results with a previous array job.

    - If a job does not complete successfuly, its SLURM logs won't be copied across to the final zarr store. For traceability, it is recommended to first move the logs from the failed jobs to a separate directory under the `store_1` root directory (for example `store_1/logs_failed`). For example, for a failed job with array job ID `2382788` and job index 3, you can run the following commands from the directory from which the `sbatch` command was run:
    ```bash
    # create a directory for the failed logs
    mkdir /path/to/store_1.zarr/logs_failed

    # copy the failed logs across
    mv slurm_array.2382788-3.gpu-380-16.* /path/to/store_1.zarr/logs_failed/
    ```
    This will copy across both `.out` and `.err` logs.

    - Edit the bash script to run on the failed jobs only. First, edit the `#SBATCH --array=...` line in the bash script to specify the failed job indices only, as a comma-separate list (e.g., `#SBATCH --array=0,5,7-9%m` for failed jobs with indices 0,5,7,8 and 9, with `m` being the maximum anumber of simultaneous jobs allowed). For more details about the syntax of the `--array` option, see the [SBATCH documentation](https://slurm.schedmd.com/sbatch.html#OPT_array).

    - Next, comment out the if-clause in the `Check inputs` section of the bash script, which throws an error if the number of input VIA track files in the provided directory matches the number of jobs in the array job. This is because we want to re-run only a subset of the jobs in the array, so the number of input files will be larger than the number of jobs and we need to skip this check.

    - Run the edited bash script with `sbatch`. If the array job runs successfully, a new zarr store that we will call `store_2` will be generated.

    - Merge the two zarr stores. To do this, we first move the video directories from `store_2` to `store_1`. This can be done using the `mv` command or drag-and-dropping the folders in a file explorer.

    - Finally, we update the metadata JSON file in the root group of `store_1` to reflect the new total number of videos processed. To do this, we can use the `zarr.consolidate_metadata` function, which updates the metadata JSON file based on the current structure of the zarr store:
    ```python
    import zarr
    zarr.consolidate_metadata("/path/to/store_1.zarr")
    ```

    - We can check use `xarray` to inspect the merged zarr store and verify that it contains the expected number of videos.
    ```python
    import xarray as xr
