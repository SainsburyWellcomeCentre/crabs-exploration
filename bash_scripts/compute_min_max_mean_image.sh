...
echo "Computing pixelwise $REDUCTION image for clip $CLIP_NAME ..."

# ----------------------------------
# Get frame indices for this clip
# ----------------------------------
# Get list of frame indices to include in the reduction AND
...
echo "Frames to include (n=${#FRAME_IDCS[@]}): $(IFS=,; echo "${FRAME_IDCS[*]}")"

# -----------------------------------------------------
# Compute pixelwise reduction image across all frames
# -----------------------------------------------------

# Run pyav command to decode the requested frames and reduce them
# pixelwise (min/max/mean) into a single image per clip
...
uv run --python 3.11 --with av,pillow,numpy - "$CLIP_PATH" "$OUTPUT_DIR" "$CLIP_NAME_NO_EXT" "$REDUCTION" "${FRAME_IDCS[@]}" <<'PYEOF'
import sys
...
reduction = sys.argv[4]
frame_idcs = [int(x) for x in sys.argv[5:]]

if reduction not in {"min", "max", "mean"}:
    raise ValueError(f"Unknown reduction '{reduction}'")

# Accumulator: a uint8 running min/max, or a float64 running sum for mean.
    ...
    for target_idx in frame_idcs:
        ...
        for frame in container.decode(stream):
            ...
            if frame.pts >= target_pts:
                ...
                # Guard against frames of differing resolution
                if ref_shape is None:
                    ref_shape = arr.shape
                elif arr.shape != ref_shape:
                    raise ValueError(
                        f"Frame at index {target_idx} has shape "
                        f"{arr.shape}, expected {ref_shape}"
                    )

                # Fold this frame into the accumulator in place,
                # so memory stays at one frame regardless of clip length.
                if reduction == "min":
...
if n_used == 0:
    raise RuntimeError(f"No frames decoded for clip {clip_name_no_ext}")
...
out_path = output_dir / f"{clip_name_no_ext}_{reduction}_n{n_used:04d}.png"
Image.fromarray(out_img).save(out_path)
print(f"Saved pixelwise {reduction} image ({n_used} frames) to {out_path}")
PYEOF
# -------------------------------------

echo "Computed pixelwise $REDUCTION image from ${#FRAME_IDCS[@]} frames of $CLIP_NAME to $OUTPUT_DIR"
