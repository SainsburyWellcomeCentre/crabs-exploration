...
def test_save_required_output(mocker, save_frames, save_video, ground_truth_dict):

    write_tracked_bbox_to_csv = mocker.patch('crabs.tracker.utils.io.write_tracked_bbox_to_csv')
    save_output_frames = mocker.patch('crabs.tracker.utils.io.save_output_frames')
    ...
    for bbox, pred_score in zip(tracked_boxes, pred_scores):
        write_tracked_bbox_to_csv.assert_any_call(
            bbox, frame, f'{video_file_root}_frame_{frame_number:08d}.png', csv_writer_mock, pred_score
        )
    if save_frames:
        save_output_frames.assert_called_once_with(
        ...
        save_output_frames.assert_not_called()
